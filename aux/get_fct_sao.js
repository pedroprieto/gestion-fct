  ...
  return axios(options)
    .then((response) => {
      if (response.statusText == "OK") {
        ...
        var options2 = {
          url:
            "https://foremp.edu.gva.es/index.php?accion=15&niaAlumno=" +
            res.nif_alumno,
          method: "GET",
          timeout: 5000,
          headers: {
    ...
    .then((response) => {
      if (response.statusText == "OK") {
        $ = cheerio.load(response.data);
        var datos_alumno = $("div.infoUsuario table.infoUsuario tbody td");
        res.nif_alumno_dni = datos_alumno.eq(2).text().trim();
        return res;
      } else {
        throw new Error("Error al obtener datos de la FCT del SAO");
      }
