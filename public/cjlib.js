...
var cjs = function() {
  ...
  function filterData(href, rel) {
    ...
    if (args === '') {
	window.location = url + "?filter=" + encodeURIComponent(href.replace(location.protocol + '//' + location.hostname + (location.port!=80 ? (':' + location.port) : '') , ''));
    } else {
	...
	url = url + "?filter=" + encodeURIComponent(href.replace(location.protocol + '//' + location.hostname + (location.port!=80 ? (':' + location.port) : ''), '') + '?');
...
window.onload = function() {
  ...
  c.g.collectionUrl = location.href.replace('index.html','') + 'users';
  c.init();
};
